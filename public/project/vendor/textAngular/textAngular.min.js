...
// Add the <style> element to the character, add as first so the styles can be overridden by custom stylesheets
...
// NOTE: here we are dealing with the html directly from the browser and not the html the player sees.
// IF you want to modify the html the player sees, do it when the player does a switchView
...
var a=B.$viewValue||"";
// if the editor isn't focused it needs to be updated, otherwise it's receiving player input
...
// make sure the end player can SEE the html code as a display. This is a read-only display element
ba(l(a)):
...
// DOM is structured in the player APP, if there is a scrollbar not as part of the
...
// Setup the default toolbar tools, this way allows the player to add new tools like plugins.
...
/* THIS is not the correct thing to do, here....
   The ngModel is correct, but it is not formatted as the player as done it...
                    var _model;
                    if (ngModel) {
...
//
updateStyles:l,
// return the current version of textAngular in use to the player
